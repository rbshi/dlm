...
class RdmaArb(cnt: Int) {
  ...
  // round-robin
  val mskLocked = RegNextWhen(mskSqSel, io.rdmaIntf.sq.fire)
  ...
  io.rdmaV(sqSel).sq >> io.rdmaIntf.sq.continueWhen(strmFifo1.io.availability > 0)

  strmFifo1.io.push.payload := sqSel
  strmFifo1.io.push.valid := io.rdmaIntf.sq.fire

  val rdSel = strmFifo1.io.pop.payload
  io.rdmaV(rdSel).rd_req << io.rdmaIntf.rd_req.continueWhen(strmFifo2.io.availability > 0)
  strmFifo2.io.push << strmFifo1.io.pop.continueWhen(io.rdmaIntf.rd_req.fire)

  val axiSrcSel = strmFifo2.io.pop.payload
  io.rdmaV(axiSrcSel).axis_src >> io.rdmaIntf.axis_src
  strmFifo2.io.pop.throwWhen(io.rdmaIntf.axis_src.fire)
  ...
  val wrReq = ReqT()
  wrReq.assignFromBits(io.rdmaIntf.wr_req.data)
  val wrSel = wrReq.vaddr.resize(log2Up(cnt) bits)

  io.rdmaV(wrSel).wr_req << io.rdmaIntf.wr_req.continueWhen(strmFifo3.io.availability > 0)
  strmFifo3.io.push.payload := wrSel
  strmFifo3.io.push.valid := io.rdmaIntf.wr_req.fire
